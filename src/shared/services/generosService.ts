import { IGeneroContenidoAudiovisual, generosContenidoAudiovisual } from "../../../database/generosContenidoAudiovisual";
import { API_CONFIG } from "../config/api";

export class GenerosService {
  static async getAll(): Promise<IGeneroContenidoAudiovisual[]> {
    try {
      const response = await fetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.GENEROS}`);
