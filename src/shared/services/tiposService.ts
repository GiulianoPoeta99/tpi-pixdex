import { ITipoContenidoAudiovisual, tiposContenidoAudiovisual } from "../../../database/tiposContenidoAudiovisual";
import { API_CONFIG } from "../config/api";

export class TiposService {
  static async getAll(): Promise<ITipoContenidoAudiovisual[]> {
    try {
      const response = await fetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.TIPOS}`);
